...
export default function AwsTemplatesPage() {
  ...
  const fetchTemplates = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await apiGet<{ data: Template[] }>("/modules/aws/templates");
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {">"} detection template gallery for aws module
          </p>
        </div>
        ...
        {showLoading || loading ? (
          ...
          <Card>
            <CardContent className="flex flex-col items-center py-12">
              ...
              <Button variant="outline" size="sm" className="mt-4" onClick={fetchTemplates}>$ retry</Button>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="flex flex-col items-center py-16">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                {categoryFilter !== "all" ? "try selecting a different category" : "no detection templates available"}
              </p>
            </CardContent>
          </Card>
        ) : (
                    ...
                    <CardContent className="flex-1 flex flex-col justify-end">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3 text-xs">
                          <span className="text-muted-foreground font-mono">cat:{template.category}</span>
                          <span className="text-muted-foreground font-mono">rules:{template.ruleCount}</span>
                        </div>
                        <button
                          ...
                          className={cn(
                            ...
                            isEnabled ? "text-primary" : "text-muted-foreground hover:text-primary",
                          )}
                        >

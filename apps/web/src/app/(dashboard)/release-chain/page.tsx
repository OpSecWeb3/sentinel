...
export default function ReleaseChainOverviewPage() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      const [imagesRes, packagesRes, eventsRes] = await Promise.allSettled([
        apiFetch<{ data: unknown[] }>("/modules/release-chain/images", {
          credentials: "include",
        }),
        apiFetch<{ data: unknown[] }>("/modules/release-chain/packages", {
          credentials: "include",
        }),
        apiFetch<{ data: unknown[]; meta?: { total?: number } }>(
          "/api/events?moduleId=release-chain&limit=50",
          { credentials: "include" },
        ),
      ]);
  ...
  const quickLinks = [
    { title: "images/", desc: "manage monitored Docker images", href: "/release-chain/images" },
    { title: "packages/", desc: "manage monitored npm packages", href: "/release-chain/packages" },
    { title: "templates/", desc: "detection template gallery", href: "/release-chain/templates" },
    { title: "events/", desc: "release chain event log", href: "/release-chain/events" },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Quick navigation */}
      <div>
        <p className="mb-2 text-xs text-muted-foreground">~/release-chain/</p>
  ...
  run: |
    curl -X POST \\
      \${{ secrets.SENTINEL_URL }}/modules/release-chain/ci/notify \\
        ...
        "repo": "\${{ github.repository }}"
            ...
            <div>
              ...
              <Button asChild variant="outline" size="sm">
                <Link href="/release-chain/templates">$ browse templates</Link>
              </Button>
            </div>
          </div>
        </CardContent>

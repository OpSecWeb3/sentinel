...
export default function PackageDetailPage() {
  ...
  async function saveCredentials() {
    setCredsSaving(true);
    try {
      await apiFetch(`/modules/registry/packages/${id}/credentials`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ npmToken }),
      });
  ...
  async function clearCredentials() {
    if (!confirm("Clear registry credentials for this package?")) return;
  ...
  return (
    <div className="space-y-6 font-mono">
        ...
        <CardContent className="p-4 space-y-4">
          ...
          <div className="space-y-3 text-xs">
            <div className="space-y-1">
              <label className="text-xs text-muted-foreground" htmlFor="npm-token">
                npm token
              </label>
              <Input
                id="npm-token"
              ...
              <Table>
                ...
                <TableHeader>
                  <TableRow className="border-b border-border hover:bg-transparent">
                    ...
                    <TableHead scope="col">Integrity</TableHead>
      ...
      <div>
        <p className="mb-3 text-xs text-muted-foreground">
          $ registry packages events ls --limit 20
          <span className="ml-2 text-foreground">
              ...
              <Table>
                ...
                <TableHeader>
                  <TableRow className="border-b border-border hover:bg-transparent">
                    ...
                    <TableHead scope="col">Hash Change</TableHead>
                              ...
                              <div className="space-y-3">
                                ...
                                {(ev.oldDigest || ev.newDigest) && (
                                  <div className="space-y-1">
                                    {ev.oldDigest && (
                                      <div className="flex items-center gap-2">
                                        <span className="w-24 text-muted-foreground">old integrity:</span>
                                        <span className="break-all font-mono text-xs text-foreground">
                                          {ev.oldDigest}
                                        </span>
                                        <button
                                          type="button"
                                          onClick={() => copyHash(ev.oldDigest!, oldKey)}
                                    ...
                                    {ev.newDigest && (
                                      <div className="flex items-center gap-2">
                                        <span className="w-24 text-muted-foreground">new integrity:</span>
                                        <span className="break-all font-mono text-xs text-foreground">
                                          {ev.newDigest}
                                        </span>
                                        <button
                                          type="button"
                                          onClick={() => copyHash(ev.newDigest!, newKey)}
                                          className="shrink-0 text-muted-foreground/50 transition-colors hover:text-primary"
                                        >

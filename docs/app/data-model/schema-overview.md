...
Stores platform user accounts. Credentials are bcrypt-hashed. Account lockout
state is tracked directly on this row.
...
| `invite_secret_hash` | `text` | Yes | Argon2/bcrypt hash of the current invite token. Used for server-side verification. |
...
Express-session store table. Managed by `connect-pg-simple`.
...
- `idx_detections_status` on `status` where `status = 'active'` — partial index; only indexes active rows, minimizing overhead.
...
| `org_id` | `uuid` | No | Denormalized org reference for query efficiency. No cascade (org deletion cascades via detection). |
...
| `last_triggered_at` | `timestamptz` | Yes | Timestamp of the most recent alert fired from this rule. Used for per-rule cooldown tracking. |
| `created_at` | `timestamptz` | No | Row creation time. |
| `updated_at` | `timestamptz` | No | Auto-updated on any change. |
...
|---|---|---|---|
| `id` | `uuid` | No | Primary key. |
| `org_id` | `uuid` | No | Owning organization. No cascade (events are immutable records). |
...
| `org_id` | `uuid` | No | Owning organization. No cascade. |
...
- `uq_alerts_event_detection_rule` (unique) on `(event_id, detection_id, rule_id)` where `event_id IS NOT NULL AND detection_id IS NOT NULL` — prevents duplicate rule-match alerts for the same event. Added in migration `0001`.
- `uq_alerts_event_correlation` (unique expression index) on `(event_id, trigger_data->>'correlationRuleId')` where `trigger_type = 'correlated' AND event_id IS NOT NULL` — prevents duplicate correlated alerts for the same event and correlation rule. This is a hand-written expression index because Drizzle ORM does not support expression indexes natively. Added in migration `0001`.
...
| `installed_by` | `uuid` | No | User who performed the OAuth installation. |
| `created_at` | `timestamptz` | No | Row creation time. |
| `updated_at` | `timestamptz` | No | Auto-updated on any change. |
...
|---|---|---|---|
| `id` | `bigint` | No | Primary key. `bigserial`. |
| `org_id` | `uuid` | No | Organization in whose context the action was taken. |
| `user_id` | `uuid` | Yes | Actor. Null for system-generated actions. |
...
| Table | Description |
|---|---|
| `github_installations` | GitHub App installation records. Each row links a GitHub organization or user account to a Sentinel org via the GitHub installation ID. |
...
| `chain_container_metrics` | Docker container CPU/memory metrics. Uses `bigserial`. |
...
| `rc_artifact_events` | Registry change events linked to the platform `events` table. |
| `rc_verifications` | Historical supply chain verification results (signature, provenance, Rekor). |
| `rc_attributions` | Links artifact changes back to the CI/CD pipeline that produced them. |
...
| `infra_ct_log_entries` | Certificate Transparency log entries from crt.sh. |
...
| `infra_finding_suppressions` | User-defined suppression rules to exclude specific findings from scoring. |
| `infra_scan_schedules` | Periodic and lightweight probe scheduling per host. |
...
| `infra_cdn_provider_configs` | Org CDN API credentials (Cloudflare, CloudFront). Credentials are AES-GCM encrypted. |
| `infra_cdn_origin_records` | Real origin IPs fetched via CDN provider APIs, stored per host. |

### AWS (`schema/aws.ts`)

| Table | Description |
|---|---|
| `aws_integrations` | AWS account or organization integrations. Supports cross-account assume-role and SQS-based CloudTrail ingestion. |
| `aws_raw_events` | Short-retention (7-day) buffer of all ingested CloudTrail events. Events matching active rules are promoted to the platform `events` table. Uses `bigserial`. |

...
describe('digestChangeEvaluator -- scenario tests', () => {
  ...
  it('2. Docker new tag "v2.0.0" appears', async () => {
    const event = makeEvent({
      eventType: 'release-chain.docker.new_tag',
      payload: {
  ...
  it('3. Docker tag removed (potential supply chain attack)', async () => {
    const event = makeEvent({
      eventType: 'release-chain.docker.tag_removed',
      payload: {
  ...
  it('5. npm version unpublished (left-pad scenario)', async () => {
    const event = makeEvent({
      eventType: 'release-chain.npm.version_unpublished',
      payload: {
        artifact: 'left-pad',
        tag: '1.0.0',
  ...
  it('6. npm maintainer changed (package takeover)', async () => {
    const event = makeEvent({
      eventType: 'release-chain.npm.maintainer_changed',
      payload: {
  ...
  it('13. Severity levels correct for each change type', async () => {
    const cases = [
      { eventType: 'release-chain.docker.tag_removed', stripped: 'tag_removed', expected: 'high' },
      { eventType: 'release-chain.npm.version_unpublished', stripped: 'version_unpublished', expected: 'high' },
      { eventType: 'release-chain.npm.maintainer_changed', stripped: 'maintainer_changed', expected: 'high' },
      { eventType: 'release-chain.docker.digest_change', stripped: 'digest_change', expected: 'medium' },
      { eventType: 'release-chain.docker.new_tag', stripped: 'new_tag', expected: 'low' },
    ];
...
describe('securityPolicyEvaluator -- scenario tests', () => {
  ...
  it('46. Unsigned npm package detection', async () => {
    ...
    // stripModulePrefix('release-chain.npm.version_published') => 'version_published'
...
describe('npmChecksEvaluator -- scenario tests', () => {
  // Note: npmChecksEvaluator handles 'release-chain.npm.version_published'
  ...
  // 'release-chain.docker.new_tag' (strips to 'new_tag') for tests that
...
describe('cross-evaluator edge cases', () => {
  ...
  it('85. digestChangeEvaluator correctly maps new_tag event type to version_published for npm', async () => {
    // Testing that release-chain.npm.version_published is in HANDLED_EVENT_TYPES

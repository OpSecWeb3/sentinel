...
const log = rootLogger.child({ component: 'release-chain-verification' });
...
// ---------------------------------------------------------------------------
// BullMQ job handler: release-chain.verify
// ---------------------------------------------------------------------------

export const verifyHandler: JobHandler = {
  jobName: 'release-chain.verify',
...
export async function enqueueVerification(params: {
  ...
  await queue.add('release-chain.verify', params, {

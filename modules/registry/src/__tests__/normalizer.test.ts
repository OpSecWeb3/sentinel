...
describe('normalizeDockerWebhook', () => {
  it('normalizes Docker Hub push to registry.docker.new_tag', () => {
    const result = normalizeDockerWebhook(baseDockerPayload(), ORG_ID, DELIVERY_ID);

    expect(result).not.toBeNull();
    expect(result!.eventType).toBe('registry.docker.new_tag');
    expect(result!.moduleId).toBe('registry');
...
describe('normalizeNpmWebhook — package:publish', () => {
  it('normalizes npm version publish correctly', () => {
    ...
    expect(result).not.toBeNull();
    expect(result!.eventType).toBe('registry.npm.version_published');
    expect(result!.moduleId).toBe('registry');
    expect(result!.orgId).toBe(ORG_ID);
    expect(result!.externalId).toBe(DELIVERY_ID);
    expect(result!.payload).toMatchObject({
...
describe('normalizeNpmWebhook — package:deprecate', () => {
  it('normalizes npm deprecation correctly', () => {
    ...
    expect(result).not.toBeNull();
    expect(result!.eventType).toBe('registry.npm.version_deprecated');
    expect(result!.payload).toMatchObject({
...
describe('normalizeNpmWebhook — maintainer change', () => {
  it('normalizes owner-added correctly', () => {
    ...
    expect(result).not.toBeNull();
    expect(result!.eventType).toBe('registry.npm.maintainer_changed');
    expect(result!.payload).toMatchObject({
      artifact: '@acme/core',
      registry: 'npmjs',
      tag: '*',
...
describe('normalizePollChange', () => {
  it('normalizes poll-detected digest change', () => {
    ...
    expect(result).not.toBeNull();
    expect(result.eventType).toBe('registry.docker.digest_change');
    expect(result.moduleId).toBe('registry');
    expect(result.orgId).toBe(ORG_ID);
    expect(result.externalId).toBeNull();
    expect(result.payload).toMatchObject({
  ...
  it('normalizes poll-detected tag removal', () => {
    ...
    expect(result.eventType).toBe('registry.docker.tag_removed');
    expect(result.payload.source).toBe('poll');
    expect((result.payload as Record<string, unknown>).artifact).toBe('library/redis');
    expect((result.payload as Record<string, unknown>).tag).toBe('alpine3.18');
  });

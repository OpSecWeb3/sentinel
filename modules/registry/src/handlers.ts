...
/**
 * Insert into rcArtifactEvents, linking a core event to the registry
...
export const webhookProcessHandler: JobHandler = {
  ...
  async process(job: Job) {
    ...
    if (
      source === 'docker' &&
      normalized.eventType === 'registry.docker.new_tag' &&
      artifactName &&
      tag
    ) {
      ...
      if (artifact) {
        ...
        if (existingVersion) {
          ...
          normalized.eventType = 'registry.docker.digest_change';
        }
      }
    }
      ...
      const attributionRules = await db
        ...
        .where(
          and(
            ...
            eq(rules.moduleId, 'registry'),
            eq(rules.ruleType, 'registry.attribution'),
...
export const verifyAggregateHandler: JobHandler = {
  jobName: 'registry.verify.aggregate',
  queueName: QUEUE_NAMES.MODULE_JOBS,
